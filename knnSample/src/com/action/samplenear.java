			...
			System.out.println("prop "+prop);
					...
					if(prop!=0){
						...
						if(prop==3)
						{
							for(int i = 0 ; i < generate.length ; i++)
							{
								if(Gasproperty.get(generate[i]).getProperty()=="中国石油")
								{
									ptemp[k++] = generate[i];
								}
							}
						...
						else if(prop==4)
						{
							for(int i = 0 ; i < generate.length ; i++)
							{
								if(Gasproperty.get(generate[i]).getProperty()=="中国石化")
								{
						...
						else if(prop==5)
						{
							for(int i = 0 ; i < generate.length ; i++)
							{
								if(Gasproperty.get(generate[i]).getProperty()=="其他")
								{
									ptemp[k++] = generate[i];
								}
							}
	...
	public void getPosition(int speed){
		...
		if(count<1){
			choiceProperty(ch_val);
				...
				long end = System.currentTimeMillis();
				totaltime+=end-start;
				System.out.println("get Position "+totaltime);
				}
	public void sendMap(){
		jsonarray.clear();
//		jsonarray.add(cur.getFront());
//		jsonarray.add(cur.getBeyond());
//		jsonarray.add(predist);
	    ...
	    double tmdist2 = Math.sqrt(Math.pow(GasPoint.point[q.getBeyond()].getX()-q.getX(),2)+Math.pow(GasPoint.point[q.getBeyond()].getY()-q.getY(), 2));
			...
			for(int i = 0 ; i < generate.length ; i++)
			{
				if(generate[i]!=-1){
					...
					double front = tmdist1+fdist[q.getFront()][generate[i]];
					double behind = tmdist2+fdist[q.getBeyond()][generate[i]];
